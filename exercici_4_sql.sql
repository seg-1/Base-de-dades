...
SELECT (SELECT DISTINCT person_name FROM tb_person AS pe WHERE pe.person_id = mp.person_id) AS person_name , COUNT(role_id) AS num_rol_movie FROM tb_movie_person AS mp GROUP BY person_id, movie_id HAVING num_rol_movie > 1;